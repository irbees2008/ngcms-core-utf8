...
* buttons The list of buttons that appear in the editor's toolbar on large places (РІвЂ°Тђ options.sizeLG).
* buttonsMD The list of buttons that appear in the editor's toolbar on medium places (РІвЂ°Тђ options.sizeMD).
* buttonsSM The list of buttons that appear in the editor's toolbar on small places (РІвЂ°Тђ options.sizeSM).
...
Allowed values are: 
* '' (an empty string) РІР‚вЂњ Indicates that content direction will be the same as either the editor UI direction or the page element direction. 
* 'ltr' РІР‚вЂњ Indicates a Left-To-Right text direction (like in English).
* 'rtl' РІР‚вЂњ Indicates a Right-To-Left text direction (like in Arabic).
...
console.log(editor.i18n('Cancel')) //Р С›РЎвЂљР С�Р ВµР Р…Р В°;

Jodit.defaultOptions.language = 'ru';
console.log(Jodit.prototype.i18n('Cancel')) //Р С›РЎвЂљР С�Р ВµР Р…Р В° 

Jodit.lang.cs = {
   Cancel: 'ZruР•РЋit'
};
Jodit.defaultOptions.language = 'cs';
console.log(Jodit.prototype.i18n('Cancel')) //ZruР•РЋit 
