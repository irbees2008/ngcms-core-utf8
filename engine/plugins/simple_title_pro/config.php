...
function about() {
	...
	$tVars = array(
		'global'  => 'Р С› Р С—Р В»Р В°Р С–Р С‘Р Р…Р Вµ',
...
function clear_cache() {
	...
	$_SESSION['simple_title_pro']['info'] = $userROW['name'] . ' Р С™РЎРЊРЎв‚¬ Р С•РЎвЂЎР С‘РЎв‚¬Р ВµР Р…';
...
function list_news() {
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_news as n ON (n.id = s.news_id) WHERE s.news_id != 0 ORDER BY s.id ASC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		...
		$tVars = array(
			...
			'del'   => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del_news&id=' . $row['sid'] . '"  /><img title="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" alt="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" src="/engine/skins/default/images/delete.gif"></a>',
		);
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => 'Р РЋР С—Р С‘РЎРѓР С•Р С” Р Р…Р С•Р Р†Р С•РЎРѓРЎвЂљР ВµР в„–',
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
...
function license() {
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => 'Р С› Р С—Р В»Р В°Р С–Р С‘Р Р…Р Вµ => Р вЂєР С‘РЎвЂ Р ВµР Р…Р В·Р С‘РЎРЏ',
...
function list_static() {
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_static as n ON (n.id = s.static_id) WHERE s.static_id != 0 ORDER BY s.id ASC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		...
		$tVars = array(
			...
			'del'   => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del_static&id=' . $row['sid'] . '"  /><img title="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" alt="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" src="/engine/skins/default/images/delete.gif"></a>',
		);
		$entries .= $xe->render($tVars);
	}
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => 'Р РЋР С—Р С‘РЎРѓР С•Р С” РЎРѓРЎвЂљР В°РЎвЂљР С‘Р С”Р С•Р Р†',
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
...
function list_cat() {
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, c.name as cname FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_category as c ON (c.id = s.cat_id) WHERE s.cat_id != 0 ORDER BY s.id ASC LIMIT ' . $start_from . ', ' . $news_per_page) as $row) {
		...
		$tVars = array(
			...
			'del'      => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del_cat&id=' . $row['sid'] . '"  /><img title="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" alt="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" src="/engine/skins/default/images/delete.gif"></a>',
		);
		$entries .= $xe->render($tVars);
	}
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => 'Р РЋР С—Р С‘РЎРѓР С•Р С” Р С”Р В°РЎвЂљР ВµР С–Р С•РЎР‚Р С‘Р в„–',
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
...
function send_title() {
	...
	switch ($_REQUEST['do']) {
		case 'cat':
			...
			if (isset($_REQUEST['submit'])) {
				...
				if (empty($title))
					$error_text[] = 'Р СћР С‘РЎвЂљР В»Р Вµ Р С•Р В±РЎРЏР В·Р В°РЎвЂљР ВµР В»РЎРЉР Р…Р В° Р Т‘Р В»РЎРЏ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С‘РЎРЏ';
				if (empty($id))
					$error_text[] = 'id Р Р…Р Вµ Р С—Р ВµРЎР‚Р ВµР Т‘Р В°Р Р…';
				if ($mysql->result('SELECT 1 FROM ' . prefix . '_simple_title_pro WHERE cat_id = \'' . $id . '\' LIMIT 1') && empty($_REQUEST['edit']))
					$error_text[] = 'Р вЂќР В»РЎРЏ РЎРЊРЎвЂљР С•Р в„– Р С”Р В°РЎвЂљР ВµР С–Р С•РЎР‚Р С‘Р С‘ РЎС“Р В¶Р Вµ Р ВµРЎРѓРЎвЂљРЎРЉ TITLE';
		...
		case 'news':
			...
			if (isset($_REQUEST['submit'])) {
				...
				if ($mysql->result('SELECT 1 FROM ' . prefix . '_simple_title_pro WHERE news_id = \'' . $id . '\' LIMIT 1') && empty($_REQUEST['edit']))
					$error_text[] = 'Р вЂќР В»РЎРЏ РЎРЊРЎвЂљР С•Р в„– Р Р…Р С•Р Р†Р С•РЎРѓРЎвЂљР С‘ РЎС“Р В¶Р Вµ Р ВµРЎРѓРЎвЂљРЎРЉ TITLE';
				if (empty($error_text) && empty($_REQUEST['edit'])) {
		...
		case 'static':
			...
			if (isset($_REQUEST['submit'])) {
				...
				if ($mysql->result('SELECT 1 FROM ' . prefix . '_simple_title_pro WHERE static_id = \'' . $id . '\' LIMIT 1') && empty($_REQUEST['edit']))
					$error_text[] = 'Р вЂќР В»РЎРЏ РЎРЊРЎвЂљР С•Р в„– РЎРѓРЎвЂљР В°РЎвЂљР С‘Р С”Р С‘ РЎС“Р В¶Р Вµ Р ВµРЎРѓРЎвЂљРЎРЉ TITLE';
				if (empty($error_text) && empty($_REQUEST['edit'])) {
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => 'Р В¤Р С•РЎР‚Р С�Р С‘РЎР‚РЎС“Р ВµР С� Р СћР С‘РЎвЂљР В»',
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
...
function list_all() {
	...
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_static as n ON (n.id = s.static_id) WHERE s.static_id != 0 ORDER BY s.id ASC') as $row) {
		...
		$tVars = array(
			...
			'cat_name_del' => '<a href="?mod=extra-config&plugin=simple_title_pro&action=del&id=' . $row['id'] . '" /><img title="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" alt="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" src="/engine/skins/default/images/delete.gif"></a>',
		);
		$entries .= $xe->render($tVars);
	}
	foreach ($mysql->select('SELECT s.id as sid, s.title as stitle, n.title as name FROM ' . prefix . '_simple_title_pro as s LEFT JOIN ' . prefix . '_news as n ON (n.id = s.news_id) WHERE s.news_id != 0 ORDER BY s.id ASC') as $row) {
		...
		$tVars = array(
			...
			'cat_name_del' => '<a href="?mod=extra-config&plugin=simple_title_pro&action=cat_name_del&id=' . $row['id'] . '"  /><img title="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" alt="Р Р€Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ" src="/engine/skins/default/images/delete.gif"></a>',
		);
		$entries .= $xe->render($tVars);
	}
	...
	$xg = $twig->loadTemplate($tpath['main'] . 'main.tpl');
	$tVars = array(
		'global'  => 'Р РЋР С—Р С‘РЎРѓР С•Р С”',
		'entries' => $xt->render($tVars)
	);
	print $xg->render($tVars);
}
...
function main() {
	...
	$tVars = array(
		'c_title'      => array(
			'print' => $c_title,
			'error' => empty($c_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> %home%  / %cat% %num%' : ''
		),
		'n_title'      => array(
			'print' => $n_title,
			'error' => empty($n_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> %home%  / %cat% / %title%  %num%' : ''
		),
		'm_title'      => array(
			'print' => $m_title,
			'error' => empty($m_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> %home% %num%' : ''
		),
		'static_title' => array(
			'print' => $static_title,
			'error' => empty($static_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> %home% / %static%' : ''
		),
		'o_title'      => array(
			'print' => $o_title,
			'error' => empty($o_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> %home% / %other% %html% %num%' : ''
		),
		'e_title'      => array(
			'print' => $e_title,
			'error' => empty($e_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> %home% / %other%' : ''
		),
		'num_title'    => array(
			'print' => $num_title,
			'error' => empty($num_title) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> / Р РЋРЎвЂљРЎР‚Р В°Р Р…Р С‘РЎвЂ Р В° %count%' : ''
		),
		'num_cat'      => array(
			'print' => $num_cat,
			'error' => empty($num_cat) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> 20' : ''
		),
		'num_news'     => array(
			'print' => $num_news,
			'error' => empty($num_news) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> 20' : ''
		),
		'num_static'   => array(
			'print' => $num_static,
			'error' => empty($num_static) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> 20' : ''
		),
		'html_secure'  => array(
			'print' => $html_secure,
			'error' => empty($html_secure) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> / %html%' : ''
		),
		'p_title'      => array(
			'print' => $p_title,
			'error' => ''
		...
		'cache'        => array(
			'print' => $cache,
			'error' => empty($cache) ? '<img src="' . skins_url . '/images/error.gif" hspace="5" alt="" />Р СџР С•Р В»Р Вµ Р Р…Р Вµ Р В·Р В°Р С—Р С•Р В»Р Р…Р ВµР Р…Р С•!<br /><b>Р В Р ВµР С�Р С•Р С�Р ВµР Р…Р Т‘Р С•Р Р†Р В°Р Р…Р Р…Р С•:</b> 1' : ''
		),
	);
	foreach ($tVars as $row) {
		//print "<pre>".var_export($row['error'], true)."</pre>";
		if (!empty($row['error'])) {
			$info .= msg(array("type" => "info", "info" => 'Р Р€ Р Р†Р В°РЎРѓ Р С•РЎв‚¬Р С‘Р В±Р С”Р В°! Р вЂўРЎРѓР В»Р С‘ Р Р…Р Вµ Р С—Р С•Р В»РЎС“РЎвЂЎР В°Р ВµРЎвЂљРЎРѓРЎРЏ Р ВµРЎвЂ� Р С‘РЎРѓР С—РЎР‚Р В°Р Р†Р С‘РЎвЂљРЎРЉ, Р С•Р В±РЎР‚Р В°РЎвЂљР С‘РЎвЂљР ВµРЎРѓРЎРЉ Р Р…Р В° РЎвЂћР С•РЎР‚РЎС“Р С� <a href="http://ngcms.ru/forum/viewtopic.php?id=2055" target="_blank"><b>simple_title_pro</b></a> Р С‘Р В»Р С‘ Р С�Р Р…Р Вµ Р Р…Р В° ICQ: 209388634 Р С‘Р В»Р С‘ jabber: rozard@ya.ru'), 0, 2);
			break;
		}
	}
	$tVars = array(
...
function del_cat() {
	...
	$id = intval($_REQUEST['id']);
	if (empty($id))
		return msg(array("type" => "error", "text" => "Р С›РЎв‚¬Р С‘Р В±Р С”Р В°, Р Р†РЎвЂ№ Р Р…Р Вµ Р Р†РЎвЂ№Р В±РЎР‚Р В°Р В»Р С‘ РЎвЂЎРЎвЂљР С• РЎвЂ¦Р С•РЎвЂљР С‘РЎвЂљР Вµ РЎС“Р Т‘Р В°Р В»Р С‘РЎвЂљРЎРЉ"));
	$mysql->query("delete from " . prefix . "_simple_title_pro where id = {$id} LIMIT 1");
	msg(array("type" => "info", "info" => "Р вЂ”Р В°Р С—Р С‘РЎРѓРЎРЉ РЎС“Р Т‘Р В°Р В»Р ВµР Р…Р В°"));
}
